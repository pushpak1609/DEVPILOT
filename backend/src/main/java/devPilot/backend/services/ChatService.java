...
public class ChatService {
    ...
    /**
     * Renames a session. Used for both manual rename and the auto-generated
     * title patched in after the first exchange (see ChatStreamHandler).
     */
    @Transactional
    public ChatSessionResponse renameSession(UUID userId, UUID sessionId, String title) {
        ChatSession session = requireSession(userId, sessionId);
    ...
    /**
     * Hard-deletes a session and all its messages. Messages are deleted
     * explicitly first so this doesn't depend on an ON DELETE CASCADE
     * existing at the DB level.
     */
    @Transactional
